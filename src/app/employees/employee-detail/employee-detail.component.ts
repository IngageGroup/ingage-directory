...
export class EmployeeDetailComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.employee = this.dataService
      .getEmployees()
      .filter((f) => f.employeeid === +empId)[0];
    this.managerLabel =
      this.employee.title === "Apprentice" ? "Mentor" : "Coach";
    this.showChampion = this.employee.title !== "Apprentice";
    this.showHBDI =
      this.employee.hbdipreference != null &&
      this.employee.hbdipreference.toLowerCase() != "n/a";
  ...
  private getPreferenceCss(preference): string {
    let pref = "";
    if (this.employee.hbdipreference) {
